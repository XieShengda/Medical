...
from tools.preprocess_utils import verify_dataset_integrity, convert_to_decathlon, crop, ExperimentPlanner2D_v21, \
    ExperimentPlanner3D_v21, DatasetAnalyzer
...
class MSDDataset(MedicalDataset):
    ...
    def preprocess_and_generate_plans(self):
        ...
        if not os.path.exists(
            ...
            modalities = list(dataset_json["modality"].values())  # 模态
            collect_intensityproperties = True if (
                    ("CT" in modalities) or ("ct" in modalities)) else False  # CT就是灰度值
